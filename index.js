...
function loadLocalFiles() {
  ...
  // Check for rehan.html
  if (fs.existsSync('./lib/rehan.html')) {
    console.log("✅ rehan.html found in lib folder");
  } else {
    console.log("⚠️ rehan.html not found in lib folder");
  }
...
// Serve HTML page
app.get('/', (req, res) => {
  const htmlPath = path.join(__dirname, 'lib', 'rehan.html');
    ...
    res.send(`
      ...
      <body>
        <div class="container">
          ...
          <p>Owner: ${config.OWNER_NAME || 'RehanMD-Official'}</p>
        </div>
      </body>
      </html>
    `);
...
app.get('/lib/rehan.html', (req, res) => {
  res.redirect('/');
});
...
async function getProfilePicture(sock, jid) {
  try {
    ...
    return 'https://ibb.co/67Q2HCxq'; // Default image
  }
}
...
async function connectToWA() {
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      const imageUrl = 'https://ibb.co/67Q2HCxq';
      ...
      for (const participant of participants) {
        ...
        if (action === 'add') {
          ...
          if (settings.goodbye) {
            try {
              // Get user's profile picture
              const ppUrl = await getProfilePicture(sock, participant).catch(() => 'https://ibb.co/67Q2HCxq');
